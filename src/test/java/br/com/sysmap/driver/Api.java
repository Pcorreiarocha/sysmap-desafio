...
public class Api {
    ...
    static String apiKey = "a79e1ceddcd96489220f727c13833f32";
...
//            System.out.println(resp);
    ...
    public static String currentWeatherUnits(String cidade, String unidade) {
        String retorno = null;
        try (CloseableHttpClient client = HttpClients.createDefault()) {
            String uri = "https://api.openweathermap.org/data/2.5/weather?" + getLatLon(cidade) + "&units=" + unidade + "&appid=" + apiKey;
            HttpGet get = new HttpGet(uri);
            HttpResponse response = client.execute(get);
            String resp = EntityUtils.toString(response.getEntity());
            JSONObject obj = new JSONObject(resp);
